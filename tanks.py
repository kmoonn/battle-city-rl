...
class Tank():
	...
	def explode(self):
		...
		if self.state != self.STATE_DEAD:
			...
			# 只有敌人有 bonus 和 spawnBonus 方法
			if hasattr(self, 'bonus') and self.bonus and hasattr(self, 'spawnBonus'):
				self.spawnBonus()
...
class Enemy(Tank):
	...
	def __init__(self, level, type, position = None, direction = None, filename = None):
		...
		if position == None:
			spawn_pos = self.getFreeSpawningPosition()
			if not spawn_pos:
				self.state = self.STATE_DEAD
				return
...
class Game():
	...
	def nextLevel(self):
		...
		def reset_for_rl(self, level_nr=1, headless=False):
			"""重置游戏到指定关卡，用于强化学习训练

			Args:
				level_nr: 关卡编号 (1-35)
				headless: 是否无头模式运行（不渲染画面）

			Returns:
				observation: 初始观察状态
			"""
			global castle, players, bullets, bonuses, enemies, labels
			...
			# 设置关卡
			self.stage = level_nr - 1  # nextLevel 会 +1
			self.headless = headless

			# 加载关卡
			self.level = Level(level_nr)
			self.timefreeze = False
			...
			if len(players) == 0:
				x = 8 * self.TILE_SIZE + (self.TILE_SIZE * 2 - 26) / 2
				y = 24 * self.TILE_SIZE + (self.TILE_SIZE * 2 - 26) / 2
				player = Player(self.level, 0, [x, y], self.DIR_UP, (0, 0, 13*2, 13*2))
				players.append(player)
			...
			# 初始步进，等待生成动画
			for _ in range(60):  # 约1秒的生成时间
				self._step_frame_internal(None)
		...
		def step_frame(self, action):
			...
			Returns:
				...
				info: 额外信息
			"""
			observation = self._step_frame_internal(action)
			...
			info = {
				'score': players[0].score if players else 0,
				'lives': players[0].lives if players else 0,
				'enemies_killed': sum(players[0].trophies.values()) - players[0].trophies["bonus"] if players else 0,
				'level_complete': self._level_complete()
			}

			return observation, reward, done, info

		def _step_frame_internal(self, action):
			"""内部步进方法"""
			global players, enemies, bullets, bonuses, labels, castle, play_sounds, sounds

			time_passed = 20  # 固定时间步进

			# 处理玩家动作
			if players and players[0].state == players[0].STATE_ALIVE and not self.game_over and self.active:
				player = players[0]
		...
		def _calculate_reward(self):
			...
			if score_diff > 0:
				reward += score_diff / 100.0  # 归一化得分奖励
			...
			# 游戏结束惩罚
			if self.game_over and not self._level_complete():
				reward -= 5.0
		...
		def _is_done(self):
			"""检查回合是否结束"""
			return self.game_over or self._level_complete()

		def _level_complete(self):
			"""检查是否通关"""
			return len(self.level.enemies_left) == 0 and len(enemies) == 0

		def get_observation(self):
			"""获取当前观察状态（符号观察）"""
			global players, enemies, bullets, castle

			obs = {
				'player': np.zeros(4, dtype=np.float32),
				'enemies': np.zeros((4, 4), dtype=np.float32),
				'bullets': np.zeros((10, 3), dtype=np.float32),
				'map': np.zeros((26, 26), dtype=np.float32),
				'enemies_remaining': np.zeros(1, dtype=np.float32)
			}

			# 玩家状态
			if players and players[0].state == players[0].STATE_ALIVE:
				p = players[0]
				obs['player'] = np.array([
					p.rect.x / 416.0,  # x 归一化
					p.rect.y / 416.0,  # y 归一化
					p.direction / 4.0,  # 方向归一化
					p.health / 100.0   # 生命值归一化
				], dtype=np.float32)

			# 敌人状态
			for i, enemy in enumerate(enemies[:4]):
				obs['enemies'][i] = np.array([
					enemy.rect.x / 416.0,
					enemy.rect.y / 416.0,
					enemy.direction / 4.0,
					(enemy.type + 1) / 4.0  # 类型归一化
				], dtype=np.float32)

			# 子弹状态
			for i, bullet in enumerate(bullets[:10]):
				obs['bullets'][i] = np.array([
					bullet.rect.x / 416.0,
					bullet.rect.y / 416.0,
			...
			for tile in self.level.mapr:
				...
				if 0 <= grid_x < 26 and 0 <= grid_y < 26:
					obs['map'][grid_y, grid_x] = tile.type / 6.0

			# 剩余敌人数量
			total_enemies = len(self.level.enemies_left) + len(enemies)
			obs['enemies_remaining'] = np.array([total_enemies / 20.0], dtype=np.float32)

			return obs

# ==================== 全局变量 ====================
# 这些变量需要在模块顶层定义，以便作为模块导入时可用
